...
CI fails if changes land under `contracts/`, `gpu-infra/`, `autoscaling/`, `gateway/`, `serving/vllm/gpu-variants/`, `serving/triton/tensorrt-llm/`, `observability/gpu-metrics/`, or `observability/cost-exporter/gpu-hour-assumptions.json` without evidence of ADR coverage (see `.github/scripts/check-adr-gate.sh`). When in doubt, add or update an ADR in the same PR.
